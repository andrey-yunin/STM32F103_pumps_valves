...
The following tasks are still pending for the Pumps & Valves Executor:

### **Phase 3: Command & Application Logic (Cont.)**

-   **Task 3.2: Implement Command Handlers**
    -   **Action:** In `task_dispatcher.c`, add `case` statements for `CMD_SET_PUMP_STATE` and `CMD_SET_VALVE_STATE`.
    -   **Action:** Parse the payload to extract `pump_id`/`valve_id` and `state`, and call `PumpsValves_SetPumpState()`/`PumpsValves_SetValveState()`.
    -   **Action:** Implement a software acknowledgment mechanism for these commands (send a response CAN message back to the Director).

### **Phase 4: Integration Testing**

-   **Task 4.1: Update Director for Testing**
    -   **Action:** Temporarily modify the Director's code (`STM32H723_mother_board`) to send `CMD_SET_PUMP_STATE` commands to Performer ID `1` (our new Executor).
-   **Task 4.2: Full Physical Test**
    -   **Action:** Connect Director and the new Executor via CAN (with transceivers). Flash firmware.
    -   **Action:** Verify the new Executor correctly controls pumps/valves based on Director commands.
