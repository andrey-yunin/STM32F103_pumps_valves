...
4. Собрать проект и пройти минимальный CAN-набор: `GET_DEVICE_INFO`, `GET_STATUS`, NACK matrix, strict DLC, broadcast/foreign address, ACK/DONE ordering.
5. Зафиксировать результат в интеграционном отчете конкретного исполнителя.
...
Для state-changing команд (`START`, `STOP`, `OPEN`, `CLOSE` и их доменные аналоги) `DONE` означает, что Executor перевел ресурс в требуемое состояние и активировал необходимые локальные защиты. `DONE` не означает завершение длительного технологического интервала рецепта. Время шага рецепта контролирует Дирижер.
...
*   **Recipe timer ownership**: длительность технологического шага ведет Дирижер. Для шага "насос включен 2000 ms" Дирижер отправляет `START`, ждет `ACK/DONE`, держит свой таймер, затем отправляет `STOP`.
*   **Resource lock**: пока ресурс занят шагом рецепта, Дирижер запрещает конфликтующие команды на тот же канал. Разрешены только диагностические запросы, штатный stop/close и аварийная recovery-политика.
