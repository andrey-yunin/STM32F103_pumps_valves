...
`DONE` подтверждает завершение обработки атомарной команды на Executor: выход переведен в требуемое состояние и, если команда включает нагрузку, safety timer успешно запущен. `DONE` не означает завершение технологического интервала рецепта. Длительность шага рецепта и момент перехода к следующему шагу контролирует Дирижер.
...
- **Recipe timing ownership**: технологическое время работы насоса/клапана ведет Дирижер. Например, для дозирования на 2000 ms Дирижер отправляет `PUMP_START`, ждет `ACK/DONE`, держит свой recipe timer, затем отправляет `PUMP_STOP`.
...
- **Fluidics heap baseline**: для текущей Fluidics-прошивки проверенная настройка `configTOTAL_HEAP_SIZE = 8192`. Значение `4096` оказалось недостаточным после включения диагностики и 16 software timers.
