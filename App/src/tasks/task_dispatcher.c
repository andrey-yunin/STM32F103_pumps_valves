 ...
 * Выполняет парсинг команд, маппинг device_id и валидацию.
 *
 *  Created on: Dec 8, 2025
 *  Refactored: Apr 7, 2026 (DDS-240 Standard)
 */
...
void app_start_task_dispatcher(void *argument)
{
	...
	for (;;)
	{
		...
		if (osMessageQueueGet(dispatcher_queueHandle, &cmd, NULL, osWaitForever) != osOK) {
			continue;
		}
		...
		osMessageQueuePut(fluidics_queueHandle, &domain_cmd, 0, 0);
	}
}
