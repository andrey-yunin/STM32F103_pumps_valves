...
void app_start_task_watchdog(void *argument)
{
	...
	/*
	 * IWDG starts before the scheduler. Refresh once when the supervisor starts,
	 * then continue only while all critical tasks report progress.
	 */
	...
	for (;;) {
		...
		/*
		 * At least one critical task stopped making progress. Put outputs into
		 * safe state and intentionally stop refreshing IWDG. Hardware reset will
		 * follow after the watchdog timeout.
		 */
		PumpsValves_AllOff();
		for (;;) {
			osDelay(APP_WATCHDOG_SUPERVISOR_PERIOD_MS);
