 ...
 * app_flash.c
 *
 * Реализация работы с Flash и идетнификацией
 *
...
void AppConfig_GetMCU_UID(uint8_t* out_uid) {
	...
	uint8_t* uid_base = (uint8_t*)0x1FFFF7E8; // Адрес UID для STM32F1
	memcpy(out_uid, uid_base, 12);
	}

/**
 * @brief Расчет контрольной суммы (CRC16-ANSI).
 */
...
/**
 * @brief Загрузка настроек по умолчанию (Заводские настройки).
 */
static void LoadDefaults(void) {
	memset(&g_app_config, 0, sizeof(AppConfig_t));
    g_app_config.magic = APP_CONFIG_MAGIC;
    g_app_config.performer_id = CAN_DEVICE_TYPE_PUMP; // По умолчанию 0x30
...
void AppConfig_FactoryReset(void) {
	if (osMutexAcquire(configMutex, osWaitForever) == osOK) {
        ...
        LoadDefaults(); // Загружаем дефолт в RAM после очистки Flash
...
uint8_t AppConfig_GetFluidicLogicalID(uint8_t physical_idx) {
	...
	if (physical_idx < 16 && osMutexAcquire(configMutex, osWaitForever) == osOK) {
		id = g_app_config.device_map[physical_idx];
		osMutexRelease(configMutex);
		}
	return id;
	}
...
void AppConfig_SetFluidicLogicalID(uint8_t physical_idx, uint8_t logical_id) {
	if (physical_idx < 16 && osMutexAcquire(configMutex, osWaitForever) == osOK) {
		g_app_config.device_map[physical_idx] = logical_id;
		osMutexRelease(configMutex);
		}
	}

uint32_t AppConfig_GetPerformerID(void) {
	uint32_t id = CAN_DEVICE_TYPE_PUMP;
	if (osMutexAcquire(configMutex, osWaitForever) == osOK) {
...
bool AppConfig_Commit(void) {
	...
	FLASH_EraseInitTypeDef EraseInitStruct;
	if (osMutexAcquire(configMutex, osWaitForever) == osOK) {
		// Обновляем контрольную сумму перед записью
		g_app_config.checksum = CalculateCRC16(&g_app_config);
